...
class MQTTChannel {
  ...
  async dispatch(topic, message) {
    const isHex = this.mqtt && this.mqtt.options && this.mqtt.options.dataFormat && this.mqtt.options.dataFormat === 'HEX';
    this[logger]('info', 'message', topic, isHex ? message.toString('hex').toUpperCase() : message.toString());
    if (this.inChainHandler) {
  ...
  async publish(topic, message, options) {
    const isHex = this.mqtt && this.mqtt.options && this.mqtt.options.dataFormat && this.mqtt.options.dataFormat === 'HEX';
    this[logger]('info', 'publish', topic, isHex ? message.toString('hex').toUpperCase() : JSON.stringify(message), options);
