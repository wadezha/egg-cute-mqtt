...
function createClient(config, app) {
  ...
  config.inMiddleware.filter(f => Object.keys(app.cuteMQTT.middleware).includes(f)).forEach(k => {
    client.useInHandler(new app.cuteMQTT.middleware[k](ctx));
  });

  config.outMiddleware.filter(f => Object.keys(app.cuteMQTT.middleware).includes(f)).forEach(k => {
    client.useOutHandler(new app.cuteMQTT.middleware[k](ctx));
  });

  client.init();

  config.topics.forEach(async (t) =>  {
    await client.channel.subscribe(t.topic, t.options);
  });
